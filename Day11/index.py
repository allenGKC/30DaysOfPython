...
def add_all_nums(*args):
    ...
    for i in args:
        if type(i) == int or type(i) == float:
            ...
            return f"{i} is not a number, Please enter only numbers"
    return sum
...
def is_valid_variable(var):
    if not var.isidentifier():
        ...
        return True
print(is_valid_variable("is_valid_variable"))
...
def most_spoken_languages(countries):
    return sorted(countries, key=lambda x: len(x['languages']), reverse=True)[:10]
